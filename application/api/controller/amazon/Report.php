...
//亚马逊订单接口类控制器
class Report extends AmazonCommonController
{
	//API 对接参数密钥信息
	private $serviceUrl = "http://mws.amazonaws.com/doc/2009-01-01/"; //亚马逊 MWS 端点（美国）
	...
	private $CreatedAfterTime = '2018-06-15T00:00:00Z'; //获取订单时间节点（创建-之后）,测试默认时间自己调
	private $CreatedBeforeTime = ''; //获取订单时间节点（之前）
	private $LastUpdatedAfterTime = '2018-07-00T00:00:00Z'; //获取订单时间节点（更新-之后）,测试默认时间自己调
	private $log_type_name = 'report'; //日志文件名称
	private $log_index = ''; //日志标识，便于查找不同店铺日志
	private $shopName = ''; //店铺名称
	private $AmazonOrderId = ''; //亚马逊订单号
	private $am_prefix = 'Report_'; //亚马逊订单号
	...
	public function ReportList(){
		//引入亚马逊客户端库
	    vendor("amazon.MarketplaceWebService.Client");
	    vendor("amazon.MarketplaceWebService.Model.GetReportListRequest");
...
// exit('AA');
			...
			$fromDate = '2018-05-00T00:00:00Z';
			// $toDate = '2018-07-11T00:00:00Z';
			$request->setAvailableFromDate($fromDate);
			// $request->setAvailableToDate($toDate);
			
			// $request->setReportTypeList(['_GET_FLAT_FILE_ALL_ORDERS_DATA_BY_LAST_UPDATE_']);
	...
	/**
	  ...
	  */
	  public function invokeGetReportList($service, $request){
	        ...
	        if(isset($ListOrdersResponse['ListOrdersResult']['NextToken'])){
	        	session_AmazonApiParam($this->am_prefix . $this->MERCHANT_ID . '.NextToken', $ListOrdersResponse['ListOrdersResult']['NextToken']);
	        }
	        session_AmazonApiParam($this->am_prefix . $this->MERCHANT_ID . '.LastUpdatedBeforeTime', $LastUpdatedBeforeTime); //将此订单最后时间保存进session，作为下一次查询开始时间。
	...
	public function invokeListOrdersByNextToken($service, $request){
		...
		if(isset($ListOrdersByNextTokenResponse['ListOrdersByNextTokenResult']['NextToken'])){
			session_AmazonApiParam($this->am_prefix . $this->MERCHANT_ID . '.NextToken', $ListOrdersByNextTokenResponse['ListOrdersByNextTokenResult']['NextToken']); //保存下一页的token
		}else{
			session_AmazonApiParam($this->am_prefix . $this->MERCHANT_ID . '.NextToken', null); //删除amazonApi.NextToken元素
		}
	...
	public function saveOrder($orders){
		...
		foreach($orders as $key => $order){
			...
			$data['fulfillment_channel'] = isset($order['FulfillmentChannel']) ? $order['FulfillmentChannel'] : ''; //订单配送方式：亚马逊配送 (AFN) 或卖家自行配送 (MFN)
	...
	public function ListOrderItems(){
		...
		foreach($shop_api as $api){
			...
			//获取当前店铺最早订单号-临时订单表
			$order = Db::name('amazon_order_temp')
						->where('shop_name', $this->shopName)
						->where("order_status='Unshipped' or order_status='PartiallyShipped' or order_status='Shipped'")
	...
	public function ListOrderItems2($orderno){
		//引入亚马逊客户端库
	    vendor("amazon.MarketplaceWebServiceOrders.Client");
	    vendor("amazon.MarketplaceWebServiceOrders.Model.ListOrderItemsRequest");
	...
	public function invokeListOrderItems($service, $request){
		...
		// $str = $this->hl_xml_string();
		// $ListOrderItemsResponse = $this->simplest_xml_to_array($str);
		// print_r($ListOrderItemsResponse);die('AAA');
